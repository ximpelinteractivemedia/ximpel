...
ximpel.View.prototype.init = function( model, el ){
	...
	// the callbacks when those events happen.
...
// 		overlayView.render();
// in order to render the view. A parentElement can be specified as an argument. By doing so, the view will be detached from any DOM 
// element that it is currently attached to and then re-attached to the parentElement.
...
ximpel.View.prototype.render = function( parentElement ){
	if( this.renderView ){
		this.renderView(); // calls the render() method on the specific view.
	}

	// If a parent is specified then detach the view and re-attach it to the specified parent element.
	if( parentElement ){
		this.detach();
